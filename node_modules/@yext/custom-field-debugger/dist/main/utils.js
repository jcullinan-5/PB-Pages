...
function randomize(v) {
  ...
  if (typeof v === "object") {
    const imageFields = ["height", "width", "url", "thumbnails"];
    if (imageFields.every((field) => Object.keys(v).includes(field))) {
      return v;
    }
    const blacklist = ["locale"];
    return Object.keys(v).reduce((out, key) => {
      const getVal = (key2, val) => {
        if (key2 === "hours") {
          return randomizeHours();
        }
        if (blacklist.includes(key2)) {
          return val;
        } else {
          return randomize(val);
        }
      };
      return {
        ...out,
        [key]: getVal(key, v[key])
      };
    }, {});
  }
}
