...
async function generateFunctionMetadata(filepath) {
  ...
  if (Node.isExportAssignment(defaultExportDeclaration)) {
    ...
    if (!entrypoint) {
      throw new Error(
        `${relativePath} contains an unsupported default export assignment. The default export must be a function, and it must be formatted as \`export default foo;\` for function \`foo\`.`
      );
    }
    ...
    if (!entrypoint) {
      throw new Error(
        `${relativePath} contains an unsupported default function declaration. The default export function must be a named function, exported as \`export default function foo(){}\` for function \`foo\``
      );
    }
    return [relativePath, { entrypoint }];
  }
  throw new Error(
    `${relativePath} does not contain a properly formatted default export. The default export must be named and declared either in the function declaration or in an \`export default ...;\` expression.`
  );
}
