          ...
          <Slider className={c({ "sm:mx-16": !props.hideArrows })}>
            {props.images.map((image, idx) => (
              <Slide index={idx} key={idx}>
                <Image className="w-284 h-284 object-cover" image={image} />
              </Slide>
            ))}
          </Slider>
            ...
            <div className="flex align-center mt-8 h-6 sm:h-1">
              {!props.hideArrows && (
                <div className="absolute left-0 sm:top-1/2 sm:-translate-y-1/2 w-full flex justify-between">
                  <ButtonBack className="w-6 h-6 sm:w-10 sm:h-10 text-black disabled:text-brand-gray-300 disabled:cursor-default">
                    {arrowSVG}
                  </ButtonBack>
                  <ButtonNext className="w-6 h-6 sm:w-10 sm:h-10 rotate-180 text-black disabled:text-brand-gray-300 disabled:cursor-default">
                    {arrowSVG}
                  </ButtonNext>
                </div>
              )}
                  ...
                  {props.images.map((_, idx) => {
                    ...
                    return (
                      <Dot
                        slide={idx}
                        key={idx}
                        className={`mx-2 basis-0 flex-grow max-w-[theme(spacing.16)] h-1 rounded-full bg-brand-gray-300 disabled:bg-pb-red-500 disabled:cursor-default ${afterStyles}`}
                      ></Dot>
                    );
                  })}
                </div>
